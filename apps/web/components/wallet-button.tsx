...
/**
 * Unified wallet button that:
 * - Auto-detects MiniPay (no popup) and uses its injected provider.
 * - Falls back to the injected connector for other wallets.
 * - Shows truncated address + chain badge when connected.
...
export function WalletButton({ className }: WalletButtonProps) {
  ...
  const handleConnect = React.useCallback(() => {
    // If MiniPay is detected, use the injected connector (no popup needed,
    // MiniPay auto-injects window.ethereum.request for eth_requestAccounts)
    if (isMiniPay) {
