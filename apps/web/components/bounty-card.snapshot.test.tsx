...
test("BountyCard snapshot", () => {
  ...
  assert.equal(
    html,
    '<article class="glow-card w-full text-card-foreground focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:ring-offset-background" data-token="cUSD"><a href="/bounty/demo" class="block p-4 outline-none sm:p-5"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><div class="flex flex-wrap items-center gap-1.5"><span class="inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide shrink-0 border-violet-500/25 bg-violet-500/10 text-violet-700 dark:text-violet-300">CODE</span><h3 class="line-clamp-1 text-base font-semibold leading-6">yeheskieltame/claudelance</h3></div><p class="mt-1 line-clamp-2 text-sm leading-5 text-muted-foreground">CI-gated · spec 0x11111111...111111 · github.com</p></div><span class="shrink-0 rounded-full border px-2.5 py-1 text-xs font-semibold text-muted-foreground">1/3</span></div><footer class="mt-4 flex items-center justify-between gap-3 text-sm"><span class="inline-flex min-w-0 items-center rounded-full border px-3 py-1 text-xs font-semibold border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-200">125 cUSD</span><span class="shrink-0 text-xs font-medium text-muted-foreground">Due in 3d 4h</span></footer></a></article>',
  );
});
